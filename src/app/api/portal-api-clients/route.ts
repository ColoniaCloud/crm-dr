...
import { generatePortalApiKey, hashPortalApiKey } from "@/lib/portal-api-auth";

const log = createLogger("api/portal-api-clients");
...
export async function GET() {
  ...
  try {
    const clients = await prisma.portalApiClient.findMany({
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const apiKeyHash = await hashPortalApiKey(apiKey);

    const client = await prisma.portalApiClient.create({
    ...
    await logOperatorAction({
      ...
      action: "CREATE_PORTAL_API_CLIENT",
      entityType: "PORTAL_API_CLIENT",
      entityId: client.id,
      description: `Creó una API key de portal de clientes para "${name}"`,
    });
