...
export default function ClientPortalApiClientsPage() {
  ...
  const fetchClients = async () => {
    setLoading(true);
    const res = await fetch("/api/portal-api-clients");
  ...
  const toggleActive = async (client: ApiClient) => {
    const res = await fetch(`/api/portal-api-clients/${client.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  if (!isSuperAdmin) {
    return <p className="text-muted-foreground text-sm">Solo SUPERADMIN puede gestionar las API keys.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">API Keys del Portal de Clientes</h1>
          <p className="text-muted-foreground text-sm">
            Claves de servidor a servidor para el sitio externo donde los clientes ven su stock, instalaciones, garantías y pagos
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-4 flex gap-2">
          <Input placeholder="Nombre del sitio/integración (ej. Portal Clientes Web)" value={newName} onChange={(e) => setNewName(e.target.value)} />
