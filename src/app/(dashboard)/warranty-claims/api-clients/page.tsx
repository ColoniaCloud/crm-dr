...
export default function WarrantyApiClientsPage() {
  ...
  const fetchClients = async () => {
    setLoading(true);
    const res = await fetch("/api/warranty-api-clients");
  ...
  const toggleActive = async (client: ApiClient) => {
    const res = await fetch(`/api/warranty-api-clients/${client.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">API Keys de Garantías</h1>
          <p className="text-muted-foreground text-sm">Claves para talleres y sitios externos que activan/consultan garantías</p>
        </div>
      </div>

      <Card>
        <CardContent className="p-4 flex gap-2">
          <Input placeholder="Nombre del partner (ej. Taller Norte)" value={newName} onChange={(e) => setNewName(e.target.value)} />
